...
function hoverSample() {
  if (!sampleHovering && flaskAdded) {
    ...
    anim
      .to("#sampleimage > small", { duration: 0.25, opacity: 0 })
      .to("#sampleimage", {
        duration: 1,
        y: -100,
        x: -230,
...
function zeroReading() {
  if (!task_done) {
    ...
    document.querySelector("#taretozero").style.visibility = "hidden";
  }
}

function common() {
  document.querySelector("#sample-reading").innerHTML = `${reading.toPrecision(
    2
  )}g`;
...
function placeFlask() {
  if (!flaskAdded) {
    var readingObj = { x: 0 };
    gsap.to("#placeflask > small", { duration: 0.25, opacity: 0 });
    ...
    document.querySelector("#taretozero").style.visibility = "visible";
